...
// Work Item Assignees junction table for multiple assignees (Epic & Feature support)
export const workItemAssignees = pgTable("work_item_assignees", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  workItemId: integer("work_item_id").notNull().references(() => workItems.id, { onDelete: 'cascade' }),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  role: varchar("role", { length: 50 }).default("ASSIGNEE"), // ASSIGNEE, OWNER, COLLABORATOR
  assignedAt: timestamp("assigned_at").defaultNow().notNull(),
  assignedBy: integer("assigned_by").references(() => users.id, { onDelete: 'set null' }),
}, (table) => {
  return {
    workItemUserIdx: uniqueIndex("work_item_assignee_unique_idx").on(table.workItemId, table.userId),
    workItemIdx: index("work_item_assignee_work_item_idx").on(table.workItemId),
    userIdx: index("work_item_assignee_user_idx").on(table.userId),
  };
});
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  workItemAssignments: many(workItemAssignees),
...
export const workItemAssigneesRelations = relations(workItemAssignees, ({ one }) => ({
  ...
  assignedByUser: one(users, {
    fields: [workItemAssignees.assignedBy],
    references: [users.id],
  }),
}));
...
export const insertWorkItemAssigneeSchema = createInsertSchema(workItemAssignees).omit({
  id: true,
  assignedAt: true,
});
...
export type InsertWorkItemAssignee = z.infer<typeof insertWorkItemAssigneeSchema>;

// Extended types with assignees for Epic and Feature
export type WorkItemWithAssignees = WorkItem & {
  assignees?: (WorkItemAssignee & { user: User })[];
};
