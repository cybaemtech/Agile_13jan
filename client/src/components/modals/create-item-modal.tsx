...
import { MultiAssigneeSelect, type AssigneeWithRole } from "@/components/ui/multi-assignee-select";
import {
  Form,
  FormControl,
  FormDescription,
...
const workItemFormSchema = z.object({
  ...
  // Multiple assignees for Epic & Feature
  multipleAssignees: z.array(z.object({
    userId: z.number(),
    user: z.any(), // User object
    role: z.enum(['OWNER', 'ASSIGNEE', 'COLLABORATOR'])
  })).optional(),
...
export function CreateItemModal({
  ...
  // State for multiple assignees (Epic & Feature support)
  const [multipleAssignees, setMultipleAssignees] = useState<AssigneeWithRole[]>([]);
  ...
  React.useEffect(() => {
    if (isOpen && currentUser && !isInitialized) {
      if (itemToEdit) {
        ...
        // Initialize multiple assignees for Epic/Feature in edit mode
        if (['EPIC', 'FEATURE'].includes(itemToEdit.type) && (itemToEdit as any).assignees) {
          const existingAssignees: AssigneeWithRole[] = (itemToEdit as any).assignees.map((assignee: any) => ({
            userId: assignee.userId,
            user: assignee.user,
            role: (assignee.role || 'ASSIGNEE') as 'OWNER' | 'ASSIGNEE' | 'COLLABORATOR'
          }));
          setMultipleAssignees(existingAssignees);
          form.setValue("multipleAssignees", existingAssignees);
        }
  ...
  React.useEffect(() => {
    if (!isOpen) {
      ...
      setMultipleAssignees([]); // Reset multiple assignees
  ...
  const onSubmit = async (data: WorkItemFormValues) => {
    ...
    try {
      ...
      const submitData: any = {
        ...
        // Multiple assignees for Epic & Feature
        multipleAssignees: ['EPIC', 'FEATURE'].includes(data.type) ? multipleAssignees : undefined,
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Conditional Assignee Field - Multiple for Epic/Feature, Single for others */}
              {['EPIC', 'FEATURE'].includes(selectedType) ? (
                <div className="space-y-4 py-3 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg border border-blue-200 p-4">
                  <FormLabel className="text-base font-semibold text-slate-700 flex items-center gap-2">
                    ...
                    Team Assignment (Professional Agile)
                  </FormLabel>
                  <div className="relative z-20 min-h-[80px]">
                    <MultiAssigneeSelect
                      value={multipleAssignees}
                      onChange={(assignees) => {
                        setMultipleAssignees(assignees);
                        form.setValue("multipleAssignees", assignees);
                      }}
